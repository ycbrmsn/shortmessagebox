 ...
 * @version 1.4.1
 * @history
 *   1.0.0 2018-01-18 完成消息盒子的最基本功能
 *   1.0.1 2018-02-02 today配置参数默认为客户端当天时间
 *   1.1.0 2018-02-02 加上搜索功能，新增删除一条消息方法
 *   1.2.0 2018-02-05 加上忽略某条消息按钮
 *   1.2.1 2018-02-05 工具栏补上名称提示title
...
;(function (factory) {
  ...
  $.fn.shortmessagebox = function (options) {
    ...
    return {
       ...
       * @param {Boolean} isAsc    [追加的消息数组是否是升序排序]
       */
      addMessages: function (messages, isAsc) {
        if (messages.push) {
          for (var i = 0; i < messages.length; i++) {
            if (isAsc) {
              // 升序
              opt.messages.splice(0, 0, messages[i]);
            } else {
              // 降序
              opt.messages.splice(0, 0, messages[messages.length - i - 1]);
            }
          }
        } else {
          opt.messages.splice(0, 0, messages);
        }
        this.refreshMessage(opt.messages);
      },
      /**
